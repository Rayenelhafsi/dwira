...
export default function PublicBienPageView({ bien, zones, backHref = '/ventes', backLabel = 'Retour a la liste', previewMode = false, onToggleVisibility, onToggleFeatureVisibility, togglingKey = null, featureReloadKey = 0 }: PublicBienPageViewProps) {
  ...
  const selectedVisibleFeatures = selectedFeatures.filter((item) => Number(item?.visibilite_client) !== 0 && String(item.onglet_id || '').trim().length > 0);
  ...
  const genericFeatureRows = selectedFeatures.filter((item) => String(item.onglet_id || '').trim().length > 0 && bien.type !== 'terrain');
  ...
  const showPublicPricingSection = false;
  const showRightColumn = showPublicPricingSection || isSectionVisible('show_modalites_paiement') || previewMode;
