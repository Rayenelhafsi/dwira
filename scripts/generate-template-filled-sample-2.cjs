...
(async () => {
  ...
  const outPath = path.join(process.cwd(), 'server', 'contracts', 'sample-contract-template-filled-v2.pdf');
  ...
  const map = {
    ...
    typeLogement: { page: 1, x: 199.9, top: 439.3, maxWidth: 180, v: 'S+3' },
    adresseBien: { page: 1, x: 240.9, top: 455.9, maxWidth: 220, v: 'Kelibia, Nabeul' },
    capacite: { page: 1, x: 221.9, top: 472.9, maxWidth: 80, v: '4' },
    adultes: { page: 1, x: 329.9, top: 471.9, maxWidth: 80, v: '2' },
    enfants: { page: 1, x: 417.9, top: 471.9, maxWidth: 80, v: '2' },
    equipementsBien: { page: 1, x: 132.9, top: 506.9, maxWidth: 420, v: "Ref REF-289, Villa S+3 Pied dans l'eau" },
    ...
    caution: { page: 2, x: 193.9, top: 453.9, maxWidth: 220, v: '1 000,00 TND' },
  ...
  const TOP_NUDGE = -8;
  const drawTop = (page, text, x, top, maxWidth = 220, size = 11) => {
    const safe = String(text || '').replace(/\u202f/g, ' ').replace(/\u00a0/g, ' ');
    const h = page.getHeight();
    let out = safe;
    while (fontRegular.widthOfTextAtSize(out, size) > maxWidth && out.length > 3) out = out.slice(0, -1);
    page.drawText(out, { x, y: h - (top + TOP_NUDGE) - size, size, font: fontRegular, color });
  };

  Object.values(map).forEach((f) => {
    const target = pages[Math.max(0, Number(f.page || 1) - 1)];
    if (!target) return;
    drawTop(target, f.v, f.x, f.top, f.maxWidth, 11);
  });

  drawTop(page3, 'Hafsi Ghaith, Responsable commercial', 336, 736, 220, 10);
